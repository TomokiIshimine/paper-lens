...
- **判定基準は `html-page-design` の `## 観点別判定基準` から担当 1 観点分を引く**: 各観点の合否閾値・チェック項目を本定義へハードコードしない。`review-aspect` で渡された 1 観点の判定基準を `html-page-design` の `## 観点別判定基準` から引いて点検する。これは生成側（`article-html-author`）と同一の真実源を共有し、生成⇄レビューの判定ズレを防ぐため（DRY。infographic-design / image-reviewer と同型）。
