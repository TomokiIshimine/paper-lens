...
| ⑥ 出力＋更新 | doc-updater | 整形出力、必要時 docs/ 追従更新 | 合格 `article.md` → `output/<記事ファイル>` |
...
- `output/<記事ファイル>` — 公開記事（恒久）。命名・体裁は `japanese-tech-writing` が規定。
- `docs/` — 構成変更時のみ追従更新（恒久）
...
- サブエージェント（`.claude/agents/`）: `paper-fetcher` / `paper-selector` / `paper-analyzer` / `article-writer` / `article-reviewer` / `doc-updater`（全 6 体、`model: opus`）
